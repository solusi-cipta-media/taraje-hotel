...
export default function OperasionalPemesananPage() {
  ...
  const handleAddGuest = () => {
    if (
      !newGuestForm.namaLengkap ||
      !newGuestForm.email ||
      !newGuestForm.telepon ||
      !newGuestForm.nomorIdentitas
    ) {
  ...
  const handleCreateBooking = () => {
    if (
      !newBookingForm.tamuId ||
      !newBookingForm.kamarId ||
      !newBookingForm.tanggalCheckIn ||
      !newBookingForm.tanggalCheckOut
    ) {
      toast.error("Mohon lengkapi semua field yang wajib diisi");
    ...
    try {
      ...
      let paymentAmount = 0;
      if (
        newBookingForm.paymentOption === "Bayar DP" ||
        newBookingForm.paymentOption === "Bayar Lunas"
      ) {
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Select
                    value={newBookingForm.tamuId}
                    onValueChange={(value) =>
                      setNewBookingForm((prev) => ({ ...prev, tamuId: value }))
                    }
                  >
                    ...
                    <DialogContent>
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <Select
                            value={newGuestForm.tipeIdentitas}
                            onValueChange={(value) =>
                              setNewGuestForm((prev) => ({
                                ...prev,
                                tipeIdentitas: value,
                        ...
                        <div className="flex gap-2 justify-end">
                          ...
                          <Button onClick={handleAddGuest}>Simpan Tamu</Button>
                        </div>
                      </div>
                    </DialogContent>
                  </Dialog>
              ...
              <div>
                ...
                {availableRooms.length === 0 ? (
                  ...
                  <Select
                    value={newBookingForm.kamarId}
                    onValueChange={(value) =>
                      setNewBookingForm((prev) => ({ ...prev, kamarId: value }))
                    }
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      {availableRooms.map((room) => {
                        const roomType = tipeKamar.find(
                          (tk) => tk.id === room.tipeKamarId
                        );
                        return (
                          <SelectItem key={room.id} value={room.id}>
                            {room.nomor} - {roomType?.nama} (
                            {formatCurrency(roomType?.hargaDefault || 0)})
                          </SelectItem>
                        );
                      })}
                    </SelectContent>
              ...
              <div className="space-y-4">
                ...
                <Select
                  value={newBookingForm.paymentOption}
                  onValueChange={(value) =>
                    setNewBookingForm((prev) => ({
                      ...prev,
                      paymentOption: value,
            ...
            {selectedBooking && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <Badge
                        variant={
                          selectedBooking.statusPemesanan === "Confirmed"
                            ? "default"
                            : "secondary"
                        }
                      >
                ...
                <div>
                  ...
                  {(() => {
                    const guest = tamu.find(
                      (t) => t.id === selectedBooking.tamuId
                    );
                    return guest ? (
                      <div className="font-medium">
                        {guest.namaLengkap}
                        <div className="text-sm text-muted-foreground">
                          {guest.email} • {guest.telepon}
                        </div>
                      </div>
                    ) : (
                ...
                <div>
                  ...
                  {(() => {
                    const room = kamar.find(
                      (k) => k.id === selectedBooking.kamarId
                    );
                    const roomType = room
                      ? tipeKamar.find((tk) => tk.id === room.tipeKamarId)
                      : null;
                    return room && roomType ? (
                      <div className="font-medium">
                        {room.nomor} - {roomType.nama}
                        <div className="text-sm text-muted-foreground">
                          {formatCurrency(roomType.hargaDefault)} per malam
                        </div>
                      </div>
                    ) : (
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    <div className="font-medium">
                      {moment(selectedBooking.tanggalCheckIn).format(
                        "DD MMM YYYY"
                      )}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="font-medium">
                      {moment(selectedBooking.tanggalCheckOut).format(
                        "DD MMM YYYY"
                      )}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="font-medium">
                      {selectedBooking.jumlahMalam} malam
                    </div>
                  </div>
                </div>
                  ...
                  <div className="p-4 bg-muted rounded-lg space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {formatCurrency(selectedBooking.totalBiaya)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>Total Terbayar:</span>
                      <span className="font-medium text-green-600">
                        {formatCurrency(selectedBooking.totalTerbayar)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>Sisa Pembayaran:</span>
                      <span className="font-medium text-red-600">
                        {formatCurrency(selectedBooking.sisaPembayaran)}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <ScrollArea className="h-32">
                    {(() => {
                      const transactions = getTransaksiByPemesanan(
                        selectedBooking.id
                      );
