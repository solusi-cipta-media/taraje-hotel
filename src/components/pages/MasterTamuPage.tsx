...
export default function MasterTamuPage() {
  ...
  const validateForm = (data: TamuFormData): boolean => {
    ...
    if (!data.email.trim()) {
      ...
      const duplicateEmail = tamu.find(
        (t) => t.email === data.email && t.id !== selectedTamu?.id
      );
      if (duplicateEmail) {
  ...
  const handleSubmit = () => {
    ...
    try {
      if (selectedTamu) {
        ...
        toast.success(
          `Data tamu "${formData.namaLengkap}" berhasil diperbarui`
        );
      } else {
        // Add new tamu
        addTamu(formData);
        toast.success(
          `Data tamu "${formData.namaLengkap}" berhasil ditambahkan`
        );
      }
      handleCloseModal();
    } catch (error) {
  ...
  const handleDelete = () => {
    ...
    try {
      ...
      if (success) {
        toast.success(
          `Data tamu "${selectedTamu.namaLengkap}" berhasil dihapus`
        );
                      ...
                      <TableCell>
                        <div className="space-y-1">
                          <Badge variant="outline">
                            {tamuItem.tipeIdentitas}
                          </Badge>
                          <div className="text-sm text-muted-foreground">
                            {tamuItem.nomorIdentitas}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    handleOpenDeleteModal(tamuItem)
                                  }
      ...
      <Dialog open={isAddEditModalOpen} onOpenChange={setIsAddEditModalOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {selectedTamu
                ? `Edit Data Tamu: ${selectedTamu.namaLengkap}`
                : "Tambah Data Tamu Baru"}
            </DialogTitle>
            <DialogDescription>
              {selectedTamu
                ? "Perbarui informasi data tamu."
                : `Kode tamu: ${generateTamuCode()}`}
            </DialogDescription>
          </DialogHeader>
