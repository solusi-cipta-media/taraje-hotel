...
export interface AppActions {
  ...
  addStaffUser: (userData: Omit<User, 'id' | 'kodePengguna' | 'tanggalDaftar'>) => void
  ...
  checkRoomAvailability: (kamarId: string, checkIn: string, checkOut: string, excludePemesananId?: string) => boolean
  getAvailableRooms: (checkIn: string, checkOut: string, tipeKamarId?: string) => Kamar[]
  ...
  getAvailableStatusOptions: (currentStatus: Kamar['status']) => Kamar['status'][]
  // Layout management actions (Admin only)
  updateLayoutLantai: (lantai: number, gridKolom: number, gridBaris: number) => void
  updateRoomPosition: (roomId: string, lantai: number, posisiLayout: number | null) => void
...
const initialState: AppState = {
  ...
  tipeKamar: [
    {
      ...
      hargaDefault: 500000,
      kapasitas: 2,
      fotoUrl: 'https://images.unsplash.com/photo-1611892440504-42a792e24d32?w=400',
      // Legacy fields for backward compatibility
      harga: 500000,
    ...
    },
    {
      ...
      hargaDefault: 750000,
      kapasitas: 3,
      fotoUrl: 'https://images.unsplash.com/photo-1566665797739-1674de7a421a?w=400',
      // Legacy fields for backward compatibility
      harga: 750000,
      fasilitas: ['AC', 'TV', 'WiFi', 'Kamar Mandi Dalam', 'Balkon', 'Mini Bar'],
    ...
    },
    {
      ...
      kapasitas: 4,
      fotoUrl: 'https://images.unsplash.com/photo-1582719478250-c89cae4dc85b?w=400',
      // Legacy fields for backward compatibility
      harga: 1200000,
      fasilitas: ['AC', 'TV', 'WiFi', 'Kamar Mandi Dalam', 'Balkon', 'Mini Bar', 'Ruang Tamu', 'Jacuzzi'],
  ...
  kamar: [
    { id: '1', nomor: '101', tipeKamarId: '1', status: 'Tersedia', lantai: 1, posisiLayout: null },
    { id: '2', nomor: '102', tipeKamarId: '1', status: 'Tersedia', lantai: 1, posisiLayout: null },
    { id: '3', nomor: '103', tipeKamarId: '2', status: 'Dibersihkan', lantai: 1, posisiLayout: null },
    { id: '4', nomor: '104', tipeKamarId: '1', status: 'Perbaikan', lantai: 1, posisiLayout: null },
    { id: '5', nomor: '201', tipeKamarId: '2', status: 'Dipesan', lantai: 2, posisiLayout: null },
    { id: '6', nomor: '202', tipeKamarId: '2', status: 'Tersedia', lantai: 2, posisiLayout: null },
    { id: '7', nomor: '203', tipeKamarId: '3', status: 'Tersedia', lantai: 2, posisiLayout: null },
    { id: '8', nomor: '301', tipeKamarId: '3', status: 'Tersedia', lantai: 3, posisiLayout: null }
  ],
  pemesanan: [
    {
      id: '1',
...
export const useAppStore = create<AppState & AppActions>((set, get) => ({
  ...
  toggleStaffStatus: (id) => {
    set(state => ({
      ...
      currentUser: state.currentUser && state.currentUser.id === id ? {
        ...state.currentUser,
        status: state.currentUser.status === 'Aktif' ? 'Tidak Aktif' : 'Aktif'
      } : state.currentUser
    }))
  },
  ...
  generateBookingCode: () => {
    ...
    const state = get()
    const todayBookings = state.pemesanan.filter(p => 
      p.kodeBooking && p.kodeBooking.includes(dateStr)
    )
  ...
  addPemesanan: (pemesanan) => {
    ...
    const newPemesanan: Pemesanan = {
      ...
      tipeKamarId: state.kamar.find(k => k.id === pemesanan.kamarId)?.tipeKamarId || '',
    ...
    set(state => ({
      pemesanan: state.pemesanan.map(p => 
        p.id === id ? { 
          ...
          status: pemesanan.statusPemesanan === 'Confirmed' ? 'Confirmed' :
                  pemesanan.statusPemesanan === 'Checked-in' ? 'CheckedIn' :
                  pemesanan.statusPemesanan === 'Checked-out' ? 'CheckedOut' :
                  pemesanan.statusPemesanan === 'Cancelled' ? 'Cancelled' : p.status
        } : p
      )
    }))
  },
  ...
  addTransaksi: (transaksi) => {
    const newTransaksi: Transaksi = {
      ...
      id: Date.now().toString(),
      // Add legacy fields for backward compatibility
      metodePembayaran: transaksi.metode === 'Transfer Bank' ? 'Transfer' :
                       transaksi.metode === 'Kartu Kredit' ? 'Card' : 'Cash',
      status: 'Success',
  ...
  updateLayoutLantai: (lantai, gridKolom, gridBaris) => {
    set(state => ({
      layoutLantai: state.layoutLantai.map(layout => 
        layout.lantai === lantai 
          ? { ...layout, gridKolom, gridBaris }
          : layout
      )
    }))
  },
  ...
  getUnpositionedRoomsForFloor: (lantai) => {
    const state = get()
    return state.kamar.filter(k => k.lantai === lantai && k.posisiLayout === null)
  }
}))